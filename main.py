...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if '/info' in msgText:
            ...
            bot.sendMessage(update.message.chat.id,info.read())
            info.close()
            return
        if '/info' in msgText:
            getUser = user_info
        ...
        if '/start' in msgText:
            ...
            start_msg += '❕Usa /tutorial y /info para mostrar la ayuda.\n'
...
def main():
    bot_token = os.environ.get('bot_token')
    #bot_token
    bot_token = '5256687985:AAHHvWDTHJtM00nSafvUKW24yiBZMHFs5SA'
